...
(function() {
  var CurlyCS, addHistory, addMultilineHandler, fs, getCommandId, merge, nodeREPL, path, ref, replDefaults, runInContext, updateSyntaxError, vm;
  ...
  replDefaults = {
    prompt: 'ccs> ',
    historyFile: (process.env.HOME ? path.join(process.env.HOME, '.ccs_history') : void 0),
  ...
  module.exports = {
    start: function(opts) {
      ...
      if (major === 0 && minor < 8) {
        console.warn("Node 0.8.0+ required for CurlyCS REPL");
        process.exit(1);
      }
      CurlyCS.register();
      process.argv = ['ccs'].concat(process.argv.slice(2));
